...
// екрана си(в ul)
function updateChat(scrollToBottom) {
	fetch(`${HOST}/getConversation.php`).
		then(response => response.json()).
		then(data => {
			renderConversation(data, scrollToBottom)
		})
}
...
function scrollToLatestMessage() {
	chatConversationElement.scrollTo(0, chatConversationElement.scrollHeight)
}

function renderConversation(conversationJSON, scrollToBottom) {
